...
check_issue_banner() {
    ...
    if grep -E -i "(\\v|\\r|\\m|\\s|$os_name)" /etc/issue.net &> /dev/null; then
        echo -e "1.7.3 Ensure remote login warning banner is configured properly --> \033[0;31mfail\033[0m"
    else
        echo -e "1.7.3 Ensure remote login warning banner is configured properly --> \033[0;32mpass\033[0m"
    fi
}
