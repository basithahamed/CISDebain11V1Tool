...
check_nodev_tmp_partition() {
    ...
    if [[ "$tmp_mount_options" == *"nodev"* ]]; then
        echo -e "1.1.2.2 Ensure nodev option set on /tmp partition --> \033[0;32mpass\033[0m"
    else
        echo -e "1.1.2.2 Ensure nodev option set on /tmp partition --> \033[0;31mfail\033[0m"
    fi
}
