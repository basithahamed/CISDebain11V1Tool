...
check_nosuid_var_log_audit() {
    if [ -d "/var/log/audit" ]; then
        ...
        if [[ $mount_options == *"nosuid"* ]]; then
            echo -e "1.1.6.4 Ensure nosuid option set on /var/log/audit partition --> \033[0;32mpass\033[0m"
        else
            echo -e "1.1.6.4 Ensure nosuid option set on /var/log/audit partition --> \033[0;31mfail\033[0m"
        fi
    else
        echo -e "1.1.6.4 Ensure nosuid option set on /var/log/audit partition --> \033[0;31mfail\033[0m (Directory not found)"
    fi
}
