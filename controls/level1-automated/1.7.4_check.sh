...
check_motd_permissions() {
    ...
    if [ -e "$motd_file" ]; then
        ...
        if [[ "$actual_uid" -eq "$expected_uid" && "$actual_gid" -eq "$expected_gid" && "$actual_permissions" -eq "$expected_permissions" ]]; then
            echo -e "1.7.4 Ensure permissions on /etc/motd are configured --> \033[0;32mpassed\033[0m"
        else
            echo -e "1.7.4 Ensure permissions on /etc/motd are configured --> \033[0;31mfailed\033[0m"
        fi
    else
        echo -e "1.7.4 Ensure permissions on /etc/motd are configured --> \033[0;32mpassed\033[0m (File not found)"
    fi
}
