...
check_reverse_path_filtering() {
    local l_output=""
    local l_output2=""
    local l_parlist=("net.ipv4.conf.all.rp_filter=1" "net.ipv4.conf.default.rp_filter=1")
    ...
    if [ -z "$l_output2" ]; then
        echo -e "3.3.7 Ensure Reverse Path Filtering is enabled --> \033[0;32mpassed\033[0m"
    else
        echo -e "3.3.7 Ensure Reverse Path Filtering is enabled --> \e[31mfailed\e[0m"
