...
check_passwd_dash_permissions() {
    ...
    if [ "$permissions" -ge 644 ] && [ "$uid" = "0/root" ] && [ "$gid" = "0/root" ]; then
        echo -e "6.1.2 Ensure permissions on /etc/passwd- are configured --> \033[0;32mPASS\033[0m"
        echo -e "$file $permissions $uid $gid"
    else
        echo -e "6.1.2 Ensure permissions on /etc/passwd- are configured --> \033[0;31mFAIL\033[0m"
        echo -e "$file $permissions $uid $gid"
    fi
}
