...
check_nodev_option_vartmp() {
    ...
    if [[ "$vartmp_mount_info" == *"nodev"* ]]; then
        echo -e "1.1.4.4 Ensure nodev option set on /var/tmp partition --> \033[0;32mpass\033[0m"
    else
        echo -e "1.1.4.4 Ensure nodev option set on /var/tmp partition --> \033[0;31mfail\033[0m"
    fi
}
