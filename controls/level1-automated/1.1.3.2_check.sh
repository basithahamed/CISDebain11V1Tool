...
check_nodev_option_var() {
    ...
    if [[ "$var_mount_options" == *"nodev"* ]]; then
        echo -e "1.1.3.2 Ensure nodev option set on /var partition --> \033[0;32mpass\033[0m"
    else
        echo -e "1.1.3.2 Ensure nodev option set on /var partition --> \033[0;31mfail\033[0m"
    fi
}
