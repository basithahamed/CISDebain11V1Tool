...
check_noexec_tmp_partition() {
    ...
    if [[ "$tmp_mount_options" == *"noexec"* ]]; then
        echo -e "1.1.2.3 Ensure noexec option set on /tmp partition --> \033[0;32mpass\033[0m"
    else
        echo -e "1.1.2.3 Ensure noexec option set on /tmp partition --> \033[0;31mfail\033[0m"
    fi
}
