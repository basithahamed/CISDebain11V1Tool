...
check_apparmor_enabled_bootloader() {
    if grep -Eqs '^\s*linux\s+/boot/vmlinuz.*\s+apparmor=1\b' /boot/grub/grub.cfg; then
        echo -e "1.6.1.2 Ensure AppArmor is enabled in the bootloader configuration --> \033[0;32mpass\033[0m"
    else
        echo -e "1.6.1.2 Ensure AppArmor is enabled in the bootloader configuration --> \033[0;31mfail\033[0m"
    fi
}
