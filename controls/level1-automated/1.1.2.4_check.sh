...
check_nosuid_tmp_partition() {
    ...
    if [[ "$tmp_mount_options" == *"nosuid"* ]]; then
        echo -e "1.1.2.4 Ensure nosuid option set on /tmp partition --> \033[0;32mpass\033[0m"
    else
        echo -e "1.1.2.4 Ensure nosuid option set on /tmp partition --> \033[0;31mfail\033[0m"
    fi
}
