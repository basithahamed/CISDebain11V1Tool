...
check_sudo_reauth_disabled() {
    ...
    if [ -n "$sudoers_with_disable_reauth" ]; then
        echo -e "\n5.3.5 Ensure re-authentication for privilege escalation is not disabled globally --> \033[0;31mFail\033[0m"
        cat "$sudoers_with_disable_reauth"
    else
        echo -e "\n5.3.5 Ensure re-authentication for privilege escalation is not disabled globally --> \033[0;32mPass\033[0m"
    fi
}
