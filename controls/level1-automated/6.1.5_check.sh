...
check_shadow_permissions() {
    ...
    if [ "$permissions" -ge 640 ] && [ "$uid" = "0/root" ] && [ "$gid" = "0/root" ]; then
        echo -e "6.1.5 Ensure permissions on /etc/shadow are configured --> \033[0;32mpassed\033[0m"
        echo -e "$file $permissions $uid $gid"
    else
        echo -e "6.1.5 Ensure permissions on /etc/shadow are configured --> \033[0;31mfailed\033[0m"
        echo -e "$file $permissions $uid $gid"
    fi
}
