...
check_nosuid_option_var() {
    ...
    if [[ "$var_mount_options" == *"nosuid"* ]]; then
        echo -e "1.1.3.3 Ensure nosuid option set on /var partition --> \033[0;32mpassed\033[0m"
    else
        echo -e "1.1.3.3 Ensure nosuid option set on /var partition --> \033[0;31mfailed\033[0m"
    fi
}
