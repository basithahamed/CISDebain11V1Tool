...
check_noexec_option_vartmp() {
    ...
    if [[ "$vartmp_mount_info" == *"noexec"* ]]; then
        echo -e "1.1.4.2 Ensure noexec option set on /var/tmp partition --> \033[0;32mpass\033[0m"
    else
        echo -e "1.1.4.2 Ensure noexec option set on /var/tmp partition --> \033[0;31mfail\033[0m"
    fi
}
