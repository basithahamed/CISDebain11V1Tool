...
check_noexec_varlog() {
    ...
    if [[ "$varlog_mount_options" == *"noexec"* ]]; then
        echo -e "1.1.5.3 Ensure noexec option set on /var/log partition --> \033[0;32mpass\033[0m"
    else
        echo -e "1.1.5.3 Ensure noexec option set on /var/log partition --> \033[0;31mfail\033[0m"
    fi
}
