...
check_nosuid_option_vartmp() {
    ...
    if [[ "$vartmp_mount_info" == *"nosuid"* ]]; then
        echo -e "1.1.4.3 Ensure nosuid option set on /var/tmp partition --> \033[0;32mpass\033[0m"
    else
        echo -e "1.1.4.3 Ensure nosuid option set on /var/tmp partition --> \033[0;31mfail\033[0m"
    fi
}
