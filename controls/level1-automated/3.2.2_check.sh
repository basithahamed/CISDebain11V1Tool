...
check_ip_forwarding_disabled() {
    {
        ...
        if [ -z "$l_output2" ]; then
            echo -e "\n3.2.2 Ensure IP forwarding is disabled --> \e[32mPASS\e[0m"
            echo -e "IP forwarding is disabled.\n"
        else
            echo -e "\n3.2.2 Ensure IP forwarding is disabled --> \e[31mFAIL\e[0m"
            echo -e "IP forwarding is enabled or not disabled.\n"
