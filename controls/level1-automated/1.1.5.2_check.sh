...
check_nodev_varlog() {
    ...
    if [[ "$varlog_mount_options" == *"nodev"* ]]; then
        echo -e "1.1.5.2 Ensure nodev option set on /var/log partition --> \033[0;32mpassed\033[0m"
    else
        echo -e "1.1.5.2 Ensure nodev option set on /var/log partition --> \033[0;31mfailed\033[0m"
    fi
}
