...
check_ignore_bogus_icmp_responses() {
    local l_output=""
    local l_output2=""
    local l_parlist="net.ipv4.icmp_ignore_bogus_error_responses=1"
    local l_searchloc="/run/sysctl.d/*.conf /etc/sysctl.d/*.conf
...
{print $2}' /etc/default/ufw)"
    ...
    if [ -z "$l_output2" ]; then
        echo -e "\n3.3.6 Ensure bogus ICMP responses are ignored --> \033[0;32mpassed\033[0m"
    else
        echo -e "\n3.3.6 Ensure bogus ICMP responses are ignored --> \e[31mfailed\e[0m"
