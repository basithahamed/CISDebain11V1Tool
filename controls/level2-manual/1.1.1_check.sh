...
check_gdm_autorun_never() {
  ...
  if check_gdm_installed; then
    [ -n "$l_output2" ] && echo -e "1.8.9 Ensure GDM autorun-never is not overridden --> \e[31mfailed\e[0m" || echo -e "1.8.9 Ensure GDM autorun-never is not overridden --> \033[0;32mpassed\033[0m"
  else
    echo -e "1.8.9 Ensure GDM autorun-never is not overridden --> \033[0;32mpassed\033[0m- GNOME Desktop Manager isn't installed\n- Recommendation is Not Applicable" 2>/dev/null
  fi
