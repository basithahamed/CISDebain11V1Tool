...
check_local_users_dot_files() {
    ...
    awk -v pat="$valid_shells" -F: '$(NF) ~ pat { print $1 " " $(NF-1) }' /etc/passwd | (
        ...
        if [ -n "$output" ]; then
            ...
            echo -e "\n6.2.17 Ensure local interactive user dot files are not group or world writable --> \e[32mPASSED\e[0m: - All user home dot files are mode: \"$maxperm\" or more restrictive\n"
        fi
    )
}
