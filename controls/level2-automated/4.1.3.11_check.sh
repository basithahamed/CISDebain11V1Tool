...
check_on_disk_session_rules() {
    ...
    if [ -n "$session_rules" ]; then
        echo -e "\n4.1.3.11 Ensure session initiation information is collected --> \e[32mpassed\e[0m\n"
        echo -e "On-disk configuration:\n$session_rules\n"
    else
        echo -e "\n4.1.3.11 Ensure session initiation information is collected --> \e[31mfailed\e[0m\n"
