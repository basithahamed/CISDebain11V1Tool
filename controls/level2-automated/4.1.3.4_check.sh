...
check_time_change_events() {
    ...
    if [ -n "$audit_rules_output" ]; then
        echo -e "\n4.1.3.4 Ensure events that modify date and time information are collected - \e[32mPass\e[0m"
        echo -e "\nResult:\n$audit_rules_output\n"
    else
        echo -e "\n4.1.3.4 Ensure events that modify date and time information are collected - \e[31mFail\e[0m"
    fi
