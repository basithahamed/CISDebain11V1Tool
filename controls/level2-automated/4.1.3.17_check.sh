...
check_on_disk_chacl_rules() {
    ...
    if [ -n "$chacl_rules" ]; then
        echo -e "\n4.1.3.17 Ensure successful and unsuccessful attempts to use the chacl command are recorded --> \e[32mpassed\e[0m\n"
        echo -e "On-disk configuration:\n$chacl_rules\n"
    else
        echo -e "\n4.1.3.17 Ensure successful and unsuccessful attempts to use the chacl command are recorded --> \e[31mfailed\e[0m\n"
