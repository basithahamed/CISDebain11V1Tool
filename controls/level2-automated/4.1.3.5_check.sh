...
check_network_events() {
    ...
    if [ -n "$audit_rules_output" ]; then
        echo -e "\n4.1.3.5 Ensure events that modify the system's network environment are collected - \e[32mpassed\e[0m"
        echo -e "\nResult:\n$audit_rules_output\n"
    else
        echo -e "\n4.1.3.5 Ensure events that modify the system's network environment are collected - \e[31mfailed\e[0m"
    fi
