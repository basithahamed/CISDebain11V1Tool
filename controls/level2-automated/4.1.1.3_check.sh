...
check_prior_auditd() {
    ...
    if [ -z "$result" ]; then
        echo -e "4.1.1.3 Ensure auditing for processes that start prior to auditd is enabled - \e[32mPass\e[0m"
    else
        echo -e "4.1.1.3 Ensure auditing for processes that start prior to auditd is enabled - \e[31mFail\e[0m"
