...
check_journald_forward_to_rsyslog() {
    ...
    if [ -n "$config_status" ]; then
        echo -e "\n4.2.2.3 Ensure journald is configured to send logs to rsyslog --> \e[32mpassed\e[0m"
    else
        echo -e "\n4.2.2.3 Ensure journald is configured to send logs to rsyslog --> \e[31mfailed\e[0m"
